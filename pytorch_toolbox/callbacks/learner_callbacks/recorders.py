...
class BaseRecorder(LearnerCallback):
    ...
    def on_train_begin(self, pbar: PBar, metrics_names: Collection[str], **kwargs) -> None:
        ...
        if hasattr(self, "_added_met_names"):
            self.names += self._added_met_names
    ...
    def on_epoch_end(
        ...
        **kwargs,
        ...
        if last_metrics is not None:
            ...
            if hasattr(self, "_added_mets"):
                last_metrics += self._added_mets
...
class Recorder(BaseRecorder):
    ...
    def on_batch_end(self, phase, **kwargs):
        ...
        self.loss_for_current_batch = self._record_loss_values_for_every_sample_in_batch(
            phase
        )
...
class TensorBoardRecorder(LearnerCallback):
    ...
    def _record_mean_losses_for_epoch(self, epoch):
        self.tb_writer.add_scalars(
            "mean_losses_for_epoch",
            self.learn.recorder.get_losses_for_epoch(epoch),
            global_step=epoch,
        )
    ...
    def _record_metrics_for_epoch(self, epoch):
        self.tb_writer.add_scalars(
            "metrics_for_epoch",
            self.learn.recorder.get_metrics_for_epoch(epoch),
            global_step=epoch,
        )
