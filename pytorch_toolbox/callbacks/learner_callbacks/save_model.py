...
class SaveModelCallback(TrackerCallback):
    ...
    def __init__(
        ...
        if self.every not in ["improvement", "epoch"]:
            logging.warning(
                f'SaveModel every {every} is invalid, falling back to "improvement".'
            )
    ...
    def on_epoch_end(self, epoch, phase, **kwargs) -> None:
        if self.every == "epoch":
            ...
            if current is None:
                logging.warning(
                    f"The key to monitor: {self.monitor} is not found in the available keys: {list(self.learn.recorder.loss_history[(phase.name, epoch)].keys())}"
                )
            else:
                if self.operator(self.best, current):
                    logging.info(
                        f"The key to monitor: {self.monitor} has value: {current} but best value is {self.best}"
                    )
                else:
                    self.best = current
                    self.learn.save_model_with_path(f"{self.save_path}")
                    logging.info(
                        f"The key to monitor: {self.monitor} has value: {current} which is the best value so the checkpoint is saved"
                    )
    ...
    def on_train_end(self, epoch, phase, **kwargs):
        ...
        if current is None:
            logging.warning(
                f"The key to monitor: {self.monitor} is not found in the available keys: {list(self.learn.recorder.loss_history[(phase.name, last_epoch)].keys())} so the best model will not be loaded"
            )
            return
        if self.every == "improvement":
            try:
                ...
                logging.info(
                    f"File at {str(self.save_path)} is not found, skip loading model"
                )
