...
from pytorch_toolbox.defaults import ModuleList, Floats, Union, Callable, Tuple, List, Any
from pytorch_toolbox.utils import listify, is_tuple
...
class OptimizerWrapper:
    ...
    def create(
        ...
        parameter_dicts = cls._construct_initial_parameter_dicts_for_trainable_parameters(
            split_groups
        )
    ...
    def step(self) -> None:
        ...
        if self.true_wd:
            for lr, wd, pg1, pg2 in zip(
                self._lr,
                self._wd,
                self.non_batch_norm_param_groups,
    ...
    def set_val(self, key: str, val: Any, bn_groups: bool = True) -> Any:
        ...
        for v, non_bn_group, bn_group in zip(
            val, self.non_batch_norm_param_groups, self.batch_norm_param_groups
        ):
            non_bn_group[key] = v
            if bn_groups:
                bn_group[key] = v
